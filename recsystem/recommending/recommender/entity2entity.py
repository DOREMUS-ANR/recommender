...
# property-specific Knowledge Graph embeddings and user feedback             #
##############################################################################
...
class Entity2Rec(Entity2Vec, Entity2Rel):
    def __init__(self, is_directed, preprocessing, is_weighted, p, q, walk_length, num_walks, dimensions, window_size,
                 workers, iterations, config, sparql, dataset, entities, default_graph, training, test, implicit,
    ...
    def _get_items_liked_by_user(self):
        ...
        with codecs.open(self.training, 'r', encoding='utf-8') as train:
            ...
            for line in train:
                ...
                if self.implicit is False and relevance >= scoreThreshold:
                    # only relevant items are used to compute the similarity, rel = 5 in a previous work
    ...
    def content_similarities(self, user, item):
        ...
        for past_item in items_liked_by_user:
            sims.append(self.relatedness_scores(past_item, item, -1))
            # append a list of property-specific scores, skip feedback
    ...
    def parse_items_rel(self, line):
        ...
        if self.implicit is False:
            relevance = 1 if relevance >= scoreThreshold else 0

        return item1, item2, relevance

    def item2id(self, item):
        global item_map_count
        if item_map.get(item):
            return item_map[item]
        item_map_count += 1
        item_id = item_map_count
        item_map[item] = item_id
        return item_id

    def write_line(self, item1, item2, relevance, file):

        file.write('%d qid:%d' % (relevance, self.item2id(item1)))

        count = 1

        # collab_score = self.collab_similarity(item1, item2)

        # file.write(' %d:%f' % (count, collab_score))

        # count += 1

        content_scores = self.relatedness_scores(item1, item2, -1)

        l = len(content_scores) - 1

        for content_score in content_scores:

            if count == l + 1:  # last score, end of line

                file.write(' %d:%f # %s\n' % (count, content_score, item2))
    ...
    def feature_generator(self):
        ...
        with codecs.open(feature_file, 'w', encoding='utf-8') as train_write:
            ...
            with codecs.open(self.training, 'r', encoding='utf-8') as training:
                for i, line in enumerate(training):
                    item1, item2, relevance = self.parse_items_rel(line)
                    if not item1:
                        continue
                    # print(item1)

                    self.write_line(item1, item2, relevance, train_write)
        ...
        #
        #     for item1 in self.items_rated_by_user_train.keys():
        #
        ...
        #
        #         candidate_items = self.get_candidates(item1)
        #
        ...
        #
        #             try:
        #                 rel = int(self.items_ratings_by_user_test[(item1, item2)])
        #                 # get the relevance score if it's in the test
        #
        #                 if self.implicit is False:
        #                     rel = 1 if rel >= scoreThreshold else 0
        #
        #             except KeyError:
        #                 rel = 0  # unrated items are assumed to be negative
        #
        #             self.write_line(item1, item2, rel, test_write)
        #
    ...
    def run(self, run_all):
        ...
        #
        # uri1 = 'http://data.doremus.org/expression/ba7a688d-9942-3a91-bc3a-4405003edbc1'
        # uri2 = 'http://data.doremus.org/expression/12202048-3cc2-3039-bc9f-67da0aab9a0d'
        # for embedding in self.embedding_files[0:-1]:
        #     # sim = embedding.most_similar(uri1)
        #     # sim = embedding.similarity(uri1, uri2)
        #
        #     for uri in embedding.wv.vocab:
        #         try:
        #             if (str(uri).startswith('http://data.doremus.org/expression/')):
        #                 print(uri)
...
if __name__ == '__main__':
    ...
    rec = Entity2Rec(args.directed, args.preprocessing, args.weighted, args.p, args.q, args.walk_length, args.num_walks,
                     ...
                     args.dataset, args.entities, args.default_graph, args.train, args.test, args.implicit,
                     args.entity_class, args.feedback_file)
